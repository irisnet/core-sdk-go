...
func ValidatePrefixedDenom(denom string) error {
	...
	if strings.TrimSpace(denomSplit[len(denomSplit)-1]) == "" {
		return sdkerrors.Wrap(sdkerrors.Register(ModuleName, 3, "invalid denomination for cross-chain transfer"), "base denomination cannot be blank")
	}
...
func ValidateIBCDenom(denom string) error {
	...
	case strings.TrimSpace(denom) == "",
		...
		return sdkerrors.Wrapf(sdkerrors.Register(ModuleName, 3, "invalid denomination for cross-chain transfer"), "denomination should be prefixed with the format 'ibc/{hash(trace + \"/\" + %s)}'", denom)
